 ...
 * Système unifié en langue française pour tous les commentaires
 */
class ChiffreAffairesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            // Données principales pour le dashboard CA
            $totalCA = DB::select("SELECT SUM(FCTV_MNT_TTC) as total FROM FACTURE_VNT WHERE FCTV_DATE BETWEEN ? AND ? AND FCTV_VALIDE = 1", [$dateDebut, $dateFin])[0]->total ?? 0;
            $totalFactures = DB::select("SELECT COUNT(*) as total FROM FACTURE_VNT WHERE FCTV_DATE BETWEEN ? AND ? AND FCTV_VALIDE = 1", [$dateDebut, $dateFin])[0]->total ?? 0;
            $moyenneFacture = $totalFactures > 0 ? $totalCA / $totalFactures : 0;
            $totalArticles = DB::select("
                SELECT SUM(fvd.FVD_QTE) as total 
            ...
            ", [$dateDebut, $dateFin])[0]->total ?? 0;

            // Evolution du CA par jour pour graphique
            $caParJour = collect(DB::select("
                SELECT CAST(FCTV_DATE as DATE) as date, SUM(FCTV_MNT_TTC) as total
                FROM FACTURE_VNT 
            ...
            // Top 10 serveurs par CA
            $topServeurs = collect(DB::select("
                SELECT TOP 10
            ...
            $caParFamille = collect(DB::select("
                SELECT 
            ...
            // CA par mode de paiement avec traduction française
            $caParMode = collect(DB::select("
                SELECT 
                    CASE 
                    ...
                    SUM(FCTV_MNT_TTC) as chiffre_affaires
                FROM FACTURE_VNT
                WHERE FCTV_DATE BETWEEN ? AND ? AND FCTV_VALIDE = 1
            ...
            return view('admin.chiffre-affaires.index', compact(
                'totalCA', 'totalFactures', 'moyenneFacture', 'totalArticles',
                'caParJour', 'topServeurs', 'caParFamille', 'caParMode',
                'dateDebut', 'dateFin'
            ));
            ...
            return view('admin.chiffre-affaires.index', [
                ...
                'totalCA' => 0, 'totalFactures' => 0, 'moyenneFacture' => 0, 'totalArticles' => 0,
                'caParJour' => collect(), 'topServeurs' => collect(), 'caParFamille' => collect(), 'caParMode' => collect(),
     ...
     * Analyse détaillée des performances par serveur avec statistiques complètes
     */
     ...
     * Analyse des ventes par catégorie avec quantités et prix moyens
     */
     ...
     * Top 30 des meilleurs clients avec historique complet des achats
     */
    public function rapportClient(Request $request)
    {
        ...
        try {
            $clients = collect(DB::select("
                ...
                WHERE fv.FCTV_DATE BETWEEN ? AND ?
                    AND fv.FCTV_VALIDE = 1
                GROUP BY c.CLT_REF, c.CLT_CLIENT, c.CLT_TELEPHONE, c.CLT_ADRESSE
                ORDER BY chiffre_affaires DESC
            ", [$dateDebut, $dateFin]));
     ...
     * Performance des utilisateurs avec statistiques de travail
     */
     ...
     * Vue d'ensemble complète pour la direction avec graphiques
     */
    ...
    /**
     * Export des rapports en différents formats
     * Gestion PDF, Excel et impression pour tous les types de rapports
     */
    public function export(Request $request)
    {
        $report = $request->input('report');
        $format = $request->input('format'); // pdf, excel, print
     ...
     * Génération de rapport PDF avec en-têtes et pieds de page
     */
    private function exportPDF($report, $dateDebut, $dateFin)
    {
        ...
        // Résumé statistique pour le rapport
     ...
     * Génération de fichier Excel avec graphiques intégrés
     */
    private function exportExcel($report, $dateDebut, $dateFin)
    {
        $data = $this->getExportData($report, $dateDebut, $dateFin);
        $filename = 'rapport_' . $report . '_' . $dateDebut . '_' . $dateFin . '.xlsx';
        
        return \Excel::download(new \App\Exports\ChiffreAffairesExport($data, $report), $filename);
    }

    /**
     * Préparation de la page d'impression
     * Interface optimisée pour l'impression avec CSS print
     */
    ...
    /**
     * Obtenir les données d'export selon le type de rapport
     * Préparation des données formatées pour les exports
     */
    private function getExportData($report, $dateDebut, $dateFin)
    {
        switch ($report) {
            case 'serveur':
                return $this->getServeursData($dateDebut, $dateFin);
            case 'famille':
                return $this->getFamillesData($dateDebut, $dateFin);
            case 'article':
                return $this->getArticlesData($dateDebut, $dateFin);
            case 'paiement':
                return $this->getPaiementsData($dateDebut, $dateFin);
            case 'client':
                return $this->getClientsData($dateDebut, $dateFin);
            case 'caissier':
                return $this->getCaissiersData($dateDebut, $dateFin);
            default:
                return [];
        }
    }
     ...
     * Données formatées pour les graphiques de tendance avec détails
     */
     ...
     * Analyse détaillée de la distribution des paiements avec statistiques
     */
    ...
    /**
     * Obtenir le top des produits les plus performants
     * Classification par chiffre d'affaires avec détails complets
     */
    ...
    /**
     * Obtenir le titre du rapport selon son type
     * Titres en français pour tous les rapports
     */
    private function getReportTitle($report)
    {
        $titles = [
            'dashboard' => 'Tableau de Bord - Chiffre d\'Affaires',
            'serveur' => 'Rapport CA par Serveur',
            'famille' => 'Rapport CA par Famille de Produits',
            ...
            'horaire' => 'Rapport CA par Tranche Horaire',
            'synthese' => 'Rapport de Synthèse Générale'
        ];
    ...
    /**
     * Obtenir les en-têtes de colonnes selon le type de rapport
     * Colonnes formatées en français pour tous les exports
     */
    private function getReportHeaders($report)
    {
        $headers = [
            'serveur' => ['Code Serveur', 'Nom Serveur', 'Nb Factures', 'CA (DH)', 'Moyenne (DH)', 'Première Vente', 'Dernière Vente'],
            'famille' => ['Famille', 'Nb Factures', 'Quantité', 'CA (DH)', 'Prix Moyen (DH)', 'Nb Articles'],
            'article' => ['Référence', 'Désignation', 'Famille', 'Quantité', 'CA (DH)', 'Prix Moyen', 'Nb Ventes'],
            'paiement' => ['Mode de Paiement', 'Nb Factures', 'CA (DH)', 'Moyenne (DH)', 'Pourcentage (%)'],
            'client' => ['Réf Client', 'Nom Client', 'Téléphone', 'Adresse', 'Nb Factures', 'CA (DH)', 'Moyenne (DH)', 'Première Visite', 'Dernière Visite'],
            'caissier' => ['Code', 'Nom Caissier', 'Nb Factures', 'CA (DH)', 'Moyenne (DH)', 'Première Vente', 'Dernière Vente', 'Jours Travaillés']
        ];
        
        return $headers[$report] ?? ['Description', 'Valeur'];
    }

    /**
     * Obtenir le résumé statistique du rapport
     * Calculs agrégés pour les résumés d'export
     */
    private function getReportSummary($report, $data)
    {
        ...
        return [
            ...
            'CA Maximum' => number_format($collection->max('chiffre_affaires'), 2, ',', ' ') . ' DH',
            'CA Minimum' => number_format($collection->min('chiffre_affaires'), 2, ',', ' ') . ' DH'
        ];
    }

    /**
     * Méthodes privées pour obtenir les données spécifiques à chaque rapport
     * Implémentation des requêtes optimisées pour les exports
     */

    private function getServeursData($dateDebut, $dateFin)
    {
        return DB::select("
            SELECT 
                ISNULL(fv.FCTV_SERVEUR, 'Non défini') as code_serveur,
    ...
    private function getFamillesData($dateDebut, $dateFin)
    {
        return DB::select("
            SELECT 
                ISNULL(f.FAM_LIB, 'Non défini') as famille,
    ...
    private function getArticlesData($dateDebut, $dateFin)
    {
        return DB::select("
            SELECT TOP 100
                a.ART_REF as reference,
    ...
    private function getPaiementsData($dateDebut, $dateFin)
    {
        return DB::select("
            SELECT 
                CASE 
    ...
    private function getClientsData($dateDebut, $dateFin)
    {
        return DB::select("
            SELECT TOP 100
                c.CLT_REF,
            ...
            INNER JOIN CLIENT c ON fv.CLT_REF = c.CLT_REF
            WHERE fv.FCTV_DATE BETWEEN ? AND ? AND fv.FCTV_VALIDE = 1
            GROUP BY c.CLT_REF, c.CLT_CLIENT, c.CLT_TELEPHONE
            ORDER BY chiffre_affaires DESC
        ", [$dateDebut, $dateFin]);
    }

    private function getCaissiersData($dateDebut, $dateFin)
    {
        return DB::select("
            SELECT 
                ISNULL(fv.FCTV_UTILISATEUR, 'Non défini') as code_caissier,
